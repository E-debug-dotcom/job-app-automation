...
function renderJobs() {
    ...
    filtered.forEach((job, index) => {
        ...
        tr.style.opacity = 0;
        tr.innerHTML = `
            <td>${highlightSearch(job.company ?? '', searchTerm)}</td>
            <td>${highlightSearch(job.location ?? '', searchTerm)}</td>
            <td>${highlightSearch(job.title ?? '', searchTerm)}</td>
            <td><a href="${job.url ?? '#'}" target="_blank" class="apply-btn">Apply</a></td>
        `;
